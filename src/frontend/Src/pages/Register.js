...
function MonthYearPicker({ currentMonth, currentYear, onSelect }) {
  ...
  const years = [];
  for (let y = 1950; y <= currentYearNow + 10; y++) {
    years.push(y);
  }
      ...
      <div className="month-grid">
        {monthNames.map((monthName, index) => (
          <button
            key={index}
            type="button"
            className={`month-cell ${index === currentMonth && selectedYear === currentYear ? 'selected' : ''}`}
